...
/* ══════════════════════════════════════
   إعدادات الصفحة الرئيسية (الصفحة التسويقية) — صور القسم "من نحن"،
   "ليه تختارنا" والبانر السفلي. مستقلة تمامًا عن صور الأقسام.
══════════════════════════════════════ */
...
function mergeHomepageSettings(raw = {}) {
  return {
    aboutImageUrl: typeof raw.aboutImageUrl === "string" ? raw.aboutImageUrl : DEFAULT_HOMEPAGE_SETTINGS.aboutImageUrl,
    choiceImageUrl: typeof raw.choiceImageUrl === "string" ? raw.choiceImageUrl : DEFAULT_HOMEPAGE_SETTINGS.choiceImageUrl,
    bannerImageUrl: typeof raw.bannerImageUrl === "string" ? raw.bannerImageUrl : DEFAULT_HOMEPAGE_SETTINGS.bannerImageUrl
  };
}
...
export const liveState = {
  ...
  homepageSettings: mergeHomepageSettings(),
  seoSettings: null,
  lastGoodAt: null,
...
export async function fetchMenuData() {
  try {
    ...
    const homepagePromise = getDoc(doc(db, "siteSettings", "homepage")).catch(err => {
      console.warn("Homepage settings unavailable; using defaults.", err);
      return null;
    });
    const homepageFallbackPromise = getDoc(doc(db, "categories", "system_homepage_settings")).catch(() => null);
    const seoPromise = getDoc(doc(db, "siteSettings", "seo")).catch(err => {
      console.warn("SEO settings unavailable; using defaults.", err);
      return null;
    });
    const [catsSnap, prodsSnap, heroSnap, heroFallbackSnap, homepageSnap, homepageFallbackSnap, seoSnap] = await Promise.all([
      getDocs(catsQ),
      getDocs(prodsQ),
      heroPromise,
      heroFallbackPromise,
      homepagePromise,
      homepageFallbackPromise,
      seoPromise
    ]);

    const categories = catsSnap.docs
      .filter(d => d.id !== "system_hero_settings" && d.id !== "system_homepage_settings")
    ...
    if (homepageSnap?.exists()) liveState.homepageSettings = mergeHomepageSettings(homepageSnap.data());
    else if (homepageFallbackSnap?.exists()) liveState.homepageSettings = mergeHomepageSettings(homepageFallbackSnap.data());
...
/* ══════════════════════════════════════
   حفظ إعدادات الصفحة الرئيسية — مستند واحد فقط siteSettings/homepage
══════════════════════════════════════ */
export async function saveHomepageSettings(settings) {
  const clean = mergeHomepageSettings(settings);
  try {
    await setDoc(doc(db, "siteSettings", "homepage"), { ...clean, updatedAt: serverTimestamp() });
  } catch (primaryErr) {
    try {
      await setDoc(doc(db, "categories", "system_homepage_settings"), {
        ...clean,
        __isHomepageSettings: true,
        visible: true,
        displayOrder: 2147483647,
        updatedAt: serverTimestamp()
      });
  ...
  liveState.homepageSettings = clean;
  return clean;
}

export function getDefaultHomepageSettings() {
  return mergeHomepageSettings();
}
