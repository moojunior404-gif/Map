...
} from "./firebase-app.js?v=20260902-40";
...
function renderHomeGrid() {
  ...
  // الصفحة الرئيسية التسويقية: 3 كروت بس، جنب بعض، وكلهم بيفتحوا الصفحة الرئيسية
  // للمنيو (مش قسم بعينه) — عشان كدا onclick بتاعهم ثابت "showMenu"
  const teaserGrid = document.getElementById('homeRedesignCategories');
  if (teaserGrid) {
    const teaser = categories.slice(0, 3);
    teaserGrid.innerHTML = teaser.map(cat => {
      const imageUrl = escapeHtml(cldOptimize(cat.imageUrl || '', 500));
      const name = escapeHtml(cat.name || '');
      return `
        <button class="home-category-card" type="button" onclick="safeInvoke(showMenu)" aria-label="فتح المنيو">
          <img src="${imageUrl}" alt="${name}" loading="lazy" decoding="async"/>
          <span class="home-category-body">
            <span class="home-category-name">${name}</span>
            <span class="home-category-meta"><span>${countLabel(cat)}</span><span class="home-category-arrow">←</span></span>
          </span>
        </button>`;
    }).join('');
    teaserGrid.querySelectorAll('img').forEach(img => setupImageLoading(img, CAT_FALLBACK));
  }

  // شبكة المنيو الفعلية جوه صفحة الكيوسك: كل الأقسام، وكل واحد بيفتح قسمه هو
  ...
  grid.innerHTML = categories.map(cat => {
    ...
    const onclick = cat.displayType === 'image'
      ? `safeInvoke(openHomepageImage,cldOptimize(decodeURIComponent('${encodedMenuUrl}'),1400),decodeURIComponent('${encodedName}'))`
      : `safeInvoke(openHomepageCategory,decodeURIComponent('${encodedId}'))`;
    return `
      <button class="home-category-card" type="button" onclick="${onclick}" aria-label="فتح ${name}">
        <img src="${imageUrl}" alt="${name}" loading="lazy" decoding="async"/>
        <span class="home-category-body">
          <span class="home-category-name">${name}</span>
          <span class="home-category-meta"><span>${countLabel(cat)}</span><span class="home-category-arrow">←</span></span>
  ...
  grid.querySelectorAll('img').forEach(img => setupImageLoading(img, CAT_FALLBACK));
...
function renderHomepageMedia() {
  const heroBg = document.getElementById('homeHeroBg');
  const c = liveState.heroSettings || {};
  const fallbackHero = 'https://raw.githubusercontent.com/anwarlebanonbakery/bakery-images/main/file_000000000ca481f4984eeedbbedbf2c1.png';
  if (heroBg) heroBg.style.backgroundImage = `url(${JSON.stringify(cldOptimize(c.imageUrl || fallbackHero, 1400))})`;

  const categories = (liveState.categories || []).filter(cat => cat.visible !== false);
  const imageUrls = categories.map(cat => cat.imageUrl).filter(Boolean);
  const heroImage = cldOptimize(c.imageUrl || fallbackHero, 1000);
  ...
  const chosenUrls = [hp.aboutImageUrl, hp.choiceImageUrl, hp.bannerImageUrl];
  const images = chosenUrls.map((url, i) => url ? cldOptimize(url, i === 2 ? 1100 : 900) : fallbackImages[i]);
